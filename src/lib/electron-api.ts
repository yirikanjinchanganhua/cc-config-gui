...
export const electronApi: ProfileService = {
    async getStore(): Promise<ProfilesStore> {
        const result = await window.electron.invoke('store:get');
        return unwrap(result as ProfilesStore | { error: string });
    },
    ...
    async updateProfile(id: string, raw: RawProfile): Promise<Profile> {
        const result = await window.electron.invoke('profiles:update', {
            id,
            ...raw,
        });
        return unwrap(result as Profile | { error: string });
    ...
    async activateProfile(id: string): Promise<Profile> {
        const result = await window.electron.invoke('profiles:activate', {
            id,
        });
        return unwrap(result as Profile | { error: string });
    },
    ...
    async testConnectivity(profileId: string): Promise<ConnectivityResult> {
        const result = await window.electron.invoke('connectivity:check', {
            profileId,
        });
    ...
    async exportProfiles(): Promise<RawProfile[]> {
        ...
        return profiles.map(({ name, apiKey, baseUrl, model }) => ({
            name,
            apiKey,
            baseUrl,
            model,
    ...
    async reorderProfiles(orderedIds: string[]): Promise<void> {
        const result = await window.electron.invoke('profiles:reorder', {
            orderedIds,
        });
        unwrap(result as null | { error: string });
    },
